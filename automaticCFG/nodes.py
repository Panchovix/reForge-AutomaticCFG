...
from ldm_patched.modules.model_patcher import set_model_options_patch_replace
from ldm_patched.ldm.modules.attention import attention_basic, attention_xformers, attention_pytorch, attention_split, attention_sub_quad, optimized_attention_for_device
...
def monkey_patching_comfy_sampling_function():
    ...
    if original_sampling_function is None:
        original_sampling_function = ldm_patched.modules.samplers.sampling_function
    # Make sure to only patch once
    if hasattr(ldm_patched.modules.samplers.sampling_function, '_automatic_cfg_decorated'):
        return
    ldm_patched.modules.samplers.sampling_function = sampling_function_patched
...
def experimental_functions(cond_input, method, exp_value, exp_normalize, pcp, psi, sigma, sigmax, attention_modifiers_input, args, model_options_copy, eval_string = ""):
    ...
    Using ldm_patched.modules.samplers.calc_cond_batch(args["model"], [args["cond_pos"], None], args["input"], args["timestep"], args["model_options"])[0]
    can work too.
